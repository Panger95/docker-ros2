...
# Default USER variable to 'root' if not set
USER=${USER:-root}
HOME=/root

# Create a non-root user if USER is not 'root'
if [ "$USER" != "root" ]; then
    echo "* enable custom user: $USER"
    useradd --create-home --shell /bin/bash --user-group --groups adm,sudo $USER

    # Check and set the default PASSWORD if not provided
    if [ -z "$PASSWORD" ]; then
    ...
    # Copy configuration files to the new user's home directory
    cp -r /root/{.config,.gtkrc-2.0,.asoundrc} ${HOME} 2>/dev/null
    chown -R $USER:$USER ${HOME}

    # Change group of '/dev/snd' if it exists
...
# Update VNC password in noVNC's ui.js
...
# Colcon and ROS setup in .bashrc
...
echo "export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp" >> $BASHRC_PATH
echo "export ROS_IP=0.0.0.0" >> $BASHRC_PATH
...
Icon=terminator
Type=Application
Categories=MATE;GTK;Utility;TerminalEmulator;System;
StartupNotify=true
...
Type=Application
Icon=firefox
Categories=MATE;GTK;Network;WebBrowser;
...
cat << EOF > $HOME/Desktop/subl.desktop
[Desktop Entry]
Name=Sublime Text
...
# Set ownership for desktop shortcuts
...
# Environment setup for MATE and ROS
echo "Setting up MATE and ROS environments..."

# Set MATE as the default desktop environment
